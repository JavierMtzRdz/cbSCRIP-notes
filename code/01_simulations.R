...
save_dir_varsel <- here("results", "simulation", "varsel")
...
fit_all_models <- function(train, x_train, y_train_cox, x_test, y_test_cox) {
    ...
    lm_ratio <- ifelse(length(y_test_cox) < ncol(x_test), 0, 0.001)
    cox_mod_path <- NULL
    tryCatch({
...
if (!is.null(fg_mod)) {
    varsel_fg <- tryCatch({
        map_df(seq_along(fg_mod$lambda), function(x) {
            coefs <- as.numeric(fg_mod$coef[, x])
            res <- as_tibble(varsel_perc(coefs, beta1))
            res %>% mutate(lambda = fg_mod$lambda[x], model = "Penalized Fine-Gray", 
                           model_size = TP + FP, 
                           exact_match = (TP == num_true && FP == 0))
        })
    }, error = function(e) NULL)
...
if (is.null(cb_mod_refitted) && !is.null(cb_mod)) {
    ...
    tryCatch({
        ...
        cb_mod_refitted <- tryCatch(
            cbSCRIP(cb_data = cb_mod$cb_data, alpha = 0.5,
                    lambda = lambda_val, coeffs = "adjusted"),
            error = function(e) NULL
        )
    }, error = function(e) {
...
cif_table <- tryCatch({
    ...
    if (!is.null(cb_mod_refitted)) {
        r_cb <- cbSCRIP:::absoluteRisk.penalizedCompRisk(
            object  = cb_mod_refitted$refitted_models,
            newdata = test,
            time    = brier_tp
        )
        r_cb_clean <- r_cb[-1, colnames(r_cb) != "time", drop = FALSE]
        risk_cb <- as.numeric(rowMeans(r_cb_clean, na.rm = TRUE))
